  ...
  return (
    <AppShell>
      <div className="p-3.5 space-y-3">
        <HeroCard eyebrow="Goals & Planning" title="Turn savings into reality" subtitle="Track every goal, every contribution">
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={s.label} className="bg-white/10 rounded-xl px-2 py-2 text-center">
                ...
                <p className="text-white font-bold text-sm leading-tight mt-0.5">{s.value}</p>
              </div>
            ))}
          </div>
        </HeroCard>
        ...
        {active.length === 0
          ...
          : active.map(g => (
            <GoalCard key={g.id} goal={g} fp={fp}
        ...
        {completed.length > 0 && (
          <>
            ...
            {completed.map(g => (
              <GoalCard key={g.id} goal={g} fp={fp} completed onContribute={() => {}} onDelete={() => store.removeGoal(g.id)} />
            ))}
          </>
        )}
      </div>
      ...
      {contributeId     && <ContributeModal   goalId={contributeId} onClose={() => setContributeId(null)} />}
    </AppShell>
  )
}

function GoalCard({ goal: g, fp, onContribute, onDelete, completed = false }: any) {
      ...
      <div className="p-4 cursor-pointer select-none" onClick={() => setExpanded(v => !v)}>
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[15px] font-bold text-green-900 leading-tight">{g.name}</p>
            {g.notes && <p className="text-xs text-green-700/50 mt-0.5 truncate">{g.notes}</p>}
            <p className="text-xs text-green-700/50 mt-1">
              {fp(g.currentAmount)} <span className="text-green-700/35">/ {fp(g.targetAmount)}</span>
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            {completed
              ? <span className="text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full">Done ✓</span>
              : <span className={clsx('text-xs font-medium', daysLeft < 30 ? 'text-amber-600' : 'text-green-700/50')}>
        ...
        <div className="border-t border-green-800/10 p-4 bg-green-50/60 space-y-3">
          <div className="grid grid-cols-2 gap-3 text-xs">
            {[
              ...
              { label: 'Remaining',      value: fp(Math.max(0, g.targetAmount - g.currentAmount)) },
          ...
          {g.contributions.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-green-700/50 mb-2">Recent contributions</p>
              <div className="space-y-0.5">
                {[...g.contributions].reverse().slice(0, 4).map((c: any) => (
                  <div key={c.id} className="flex justify-between py-1.5 border-b border-green-800/[0.07] last:border-0">
                    <span className="text-xs text-green-700/70">
                      {c.note || 'Contribution'} · {formatDate(c.date)}
                    </span>
                    <span className="text-xs font-bold text-green-800">+{fp(c.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-2 pt-1">
            {!completed && (
              <button onClick={onContribute} className="btn-primary flex-1 h-10 text-xs flex items-center justify-center gap-1">
...
function AddGoalModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = () => {
    ...
    store.addGoal({ name: name.trim(), targetAmount: parseFloat(target), startDate, targetDate, notes, color })
    onClose()
  }
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div className="space-y-3">
        <div><label className="label">Goal name</label><input className="input" placeholder="e.g. Emergency Fund" value={name} onChange={e => setName(e.target.value)} /></div>
        <div><label className="label">Target amount</label><input className="input" type="number" placeholder="100000" inputMode="numeric" value={target} onChange={e => setTarget(e.target.value)} /></div>
        <div className="grid grid-cols-2 gap-2">
          ...
          <div><label className="label">Target date</label><input className="input" type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)} /></div>
        </div>
        <div><label className="label">Notes</label><input className="input" placeholder="What is this goal for?" value={notes} onChange={e => setNotes(e.target.value)} /></div>
        <div>
          <label className="label">Color</label>
          <div className="flex gap-2 mt-1">
            {GOAL_COLORS.map(c => (
              <button key={c} onClick={() => setColor(c)}
                className={clsx('w-8 h-8 rounded-full border-2 transition-all', color === c ? 'border-green-900 scale-110' : 'border-transparent')}
                style={{ background: c }}
              />
            ))}
          </div>
      ...
      <div className="flex gap-2 mt-5">
        ...
        <button onClick={submit} className="btn-primary flex-1 h-12">Create goal</button>
      </div>
    </Modal>
  )
}
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <p className="text-sm text-green-700/60 mb-4">
        Adding to: <span className="font-semibold text-green-800">{goal.name}</span>
        <span className="text-green-700/40 ml-1">· {fp(Math.max(0, goal.targetAmount - goal.currentAmount))} remaining</span>
      </p>
      <div className="space-y-3">
        ...
        <div><label className="label">Note (optional)</label><input className="input" placeholder="Monthly kurot" value={note} onChange={e => setNote(e.target.value)} /></div>
      </div>
      <div className="flex gap-2 mt-5">
        <button onClick={onClose} className="h-12 px-5 text-sm border border-green-800/20 rounded-2xl text-green-700/70 font-semibold">Cancel</button>
        <button onClick={submit} className="btn-primary flex-1 h-12">Add contribution</button>
      </div>
    </Modal>
  )
}
