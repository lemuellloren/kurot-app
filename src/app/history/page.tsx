    ...
    <AppShell>
      <div className="p-3.5 space-y-3">
        ...
        <div className="flex justify-between items-center px-1">
          ...
          <button onClick={saveSnapshot} className="btn-secondary text-xs py-1.5 px-3">
            Save This Month
          </button>
        </div>
        ...
        {history.length === 0 ? (
          <div className="card p-8 text-center">
            ...
            <p className="text-xs text-green-700/35 mt-1">Save a snapshot to start tracking progress.</p>
          </div>
        ) : (
          history.map(h => {
            const ts  = h.snapshots.reduce((a, e) => a + e.spent, 0)
            const max = Math.max(...h.snapshots.map(e => e.spent), 1)
            const savingsRate = h.income > 0 ? Math.max(0, Math.round((h.income - ts) / h.income * 100)) : 0
            return (
              <div key={h.month} className="card p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="text-[15px] font-bold text-green-900">{h.month}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-bold text-green-800">{fp(h.income)}</p>
                  </div>
                </div>
